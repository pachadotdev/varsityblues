...
title: 'My Assignment'
author: 'You R. Name'
date: 'Subject to change - Last major revision: September 15, 2021'
output:
...
csl: ../csl/chicago.csl
# This will automatically install the {remotes} package and {varsityblues}
# Change this to FALSE if you'd like to install them manually on your own.
params:
  'Install required packages for {varsityblues}': True
---
...
<!--
You can delete this chunk afterwards, but Knit without removing it first to setup LaTeX. On Windows, you might need to restart your computer and try again if knitting the document doesn't work.
-->
```{r latex_setup}
...
Let $K$ be a field of \dfn{scalars}\index{field!scalar}\index{scalar}---usually either the real numbers $\R$ or the complex numbers 
$\mathbb C$, or occasionally the rationals $\mathbb Q$. A \idfn{vector space} over $K$ is a set
$V$ of \dfn{vectors}\index{vector} equipped with two operations, vector addition $(x,y)\mapsto x+y$, and scalar multiplication $(\alpha,x)\mapsto \alpha x$, where $x,y\in V$ and $\alpha\in K$.  The operations satisfy:
